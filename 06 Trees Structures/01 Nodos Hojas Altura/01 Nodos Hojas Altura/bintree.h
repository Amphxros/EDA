...
public:
    ...
    //cuenta el número de nodos del arbol
    int nodos() {
        int numNodos = 0;
        cuentaNodos(raiz, numNodos);
        return numNodos;
    }
    void cuentaNodos(Link a, int& numNodos) {
        if (a != nullptr) {
            numNodos++;
            cuentaNodos(a->left, numNodos);
            cuentaNodos(a->right, numNodos);
        }
    }
    //Cuenta las hojas del arbol
    int hojas() {
        int numHojas = 0;
        cuentaHojas(raiz, numHojas);
        return numHojas;
    }
    void cuentaHojas(Link a, int& numHojas) {
        if (a != nullptr) {
            if (a->left == nullptr && a->right == nullptr)numHojas++;
            else {
                cuentaHojas(a->left, numHojas);
                cuentaHojas(a->right, numHojas);
            }
        }
    }
    ...
    int altura() {
        int alto = 0;
        int max = 0;
        calculAlt(raiz, alto, max);
        return max;
    }
    void calculAlt(Link a, int alto, int& max) {
        if (a != nullptr) {
            calculAlt(a->left, alto + 1, max);
            calculAlt(a->right, alto + 1, max);
        }
        else {
